import {SortTypes, DEFAULT_SORT} from '../const.js';
...
function createSortItemTemplate(type) {
  ...
  const isDisabled = type === SortTypes.EVENT || type === SortTypes.OFFERS ? 'disabled' : '';

  return (
    `<div class="trip-sort__item  trip-sort__item--${type}">
      <input id="sort-${type}" class="trip-sort__input  visually-hidden" type="radio" name="trip-sort" data-sort="${type}" value="sort-${type}" ${isChecked} ${isDisabled}>
...
export default class SortListView extends AbstractView {
  ...
  #sortTypeChangeHandler = (event) => {
    ...
    const targetSort = event.target.getAttribute('data-sort');
    this.#onSortTypeChange(targetSort);
  };
}
